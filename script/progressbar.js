...
function project(x) {
  if (game.OP >= V(game.factorBoosts + 3, 1) && game.bulkBoost == 0) {
    ...
    if(game.ord<1e270){
	    ...
	    let goal = game.factorShifts==7?(2.22e222+2.22e212+2.22e202):factorShiftCosts[game.factorShifts]/OPmult
	    let ordGoal = OPtoOrd(goal,game.base)
	    percent = Math.min(1,ord / ordGoal)*100
	    //console.log((ordGoal-ord)/(rate))
	    get("factorBoostProg").style.width=percent + "%"
	    get("factorBoostProg").textContent=percent.toFixed(2) + "%"
	    get("nextBulkTime").textContent=percent==100?"Goal reached!":("The next "+(game.factorShifts==7?"layer":"Factor Shift")+" will take " + time((ordGoal-ord)/(rate)) + " assuming your Tier 1 speed stays constant")
	}else{
	    get("nextBulkTime").innerHTML =
...
function challengeProject() {
  ...
  if (game.chal9==1||(game.challenge>0&&game.challengeCompletion[game.challenge-1]>=3)) {
    ...
    let ordGoal = OPtoOrd(goal,game.base)
    percent = Math.min(1,ord / ordGoal)*100
    if((ordGoal-ord)/(rate)>1e10){
	    let goal1 = factorShiftCosts[game.factorShifts]/OPmult
	    let ordGoal1 = OPtoOrd(goal1,game.base)
	    percent1 = Math.min(1,ord / ordGoal1)*100
	    get("chalProg").style.width=percent1 + "%"
	    get("chalProg").textContent=percent1.toFixed(2) + "%"
	    get("nextChalBulkTime").textContent=percent1==100?"Maybe you need to Factor Shift":"The next Factor Shift will take " + time((ordGoal1-ord)/(rate)) + " assuming your Tier 1 speed stays constant"
    }else{
	    get("chalProg").style.width=percent + "%"
	    get("chalProg").textContent=percent.toFixed(2) + "%"
